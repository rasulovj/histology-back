...
class PremiumExpiryTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def _insert_payment(self, user_id: str, paid_at: datetime):
        async with aiosqlite.connect(user_service.DB_PATH) as db:
            await db.execute(
                "INSERT INTO payment_history (user_id, tx_id, amount, paid_at) VALUES (?, ?, ?, ?)",
                (user_id, f"tx-{paid_at.timestamp()}", 10000, paid_at.strftime("%Y-%m-%d %H:%M")),
            )
            await db.commit()
